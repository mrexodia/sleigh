...
class PcodePrinter : public ghidra::PcodeEmit {
  std::ostream &s;
  ghidra::Address instructionAddr;
  uint32_t pcodeAddr = 0;

public:
  PcodePrinter(std::ostream &s) : s(s) {}

  void dump(const ghidra::Address &addr, ghidra::OpCode op,
...
private:
  void PrintVarData(ghidra::OpCode op, ghidra::VarnodeData &data) {
    auto space = data.space;
    const auto &name = space->getName();
    if (op == ghidra::OpCode::CPUI_BRANCH ||
        op == ghidra::OpCode::CPUI_CBRANCH && name == "const") {
      s << '(' << name << ',';
      auto rel = (ghidra::intb)data.offset;
      if (rel < 0) {
...
static void PrintPcode(ghidra::Sleigh &engine, uint64_t addr, size_t len) {
  ...
  while (cur_addr < last_addr) {
    std::cout << "=> ";
    engine.printAssembly(asm_emit, cur_addr);
...
class HaltCallBack : public ghidra::BreakCallBack {
  bool pcodeCallback(ghidra::PcodeOpRaw *op) override {
    emulate->setHalt(true);
    return true;
  }
  bool addressCallback(const ghidra::Address &addr) override {
    emulate->setHalt(true);
    return true;
  }
};

static void EmulatePcode(ghidra::Sleigh &engine, uint64_t addr, size_t len,
  ...
  while (!emulator.getHalt()) {
    ...
    if (trace) {
      ...
      engine.printAssembly(asm_emit, addr);
    }
